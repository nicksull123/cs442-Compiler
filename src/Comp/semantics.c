...
void
doDeclare(char *name, struct VarType *type, int arg, int size)
{
    struct SymEntry *ent;
    type->isArg = arg;
    type->SPos = sPos;
...
doAssign(struct IdAddr *addr, struct ExprRes *Expr, int inverse)
{
    ...
    AppendSeq(code, GenInstr(NULL, "sw", TmpRegName(Expr->Reg),
                        RegOff(0, TmpRegName(addrExpr->Reg)), NULL));
...
Finish(struct InstrSeq *Code)
{
    ...
    while (entry)
    {
        ...
        if (vType->Shim)
        {
            AppendSeq(code, GenInstr((char *)GetName(entry), ".word",
                                vType->Shim, NULL, NULL));
        }
        else
        {
            snprintf(buf, 50, "%d", vType->Size * 4);
            AppendSeq(code,
                GenInstr((char *)GetName(entry), ".space", buf, NULL, NULL));
        }
    ...
    while (strEnt)
    {
        AppendSeq(code, GenInstr(NULL, ".align", "4", NULL, NULL));
        AppendSeq(
            code, GenInstr(strEnt->label, ".asciiz", strEnt->val, NULL, NULL));
        strEnt = strEnt->next;
    }
