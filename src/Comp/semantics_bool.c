...
doNot(struct ExprRes *Expr)
{
    ...
    AppendSeq(instrs, GenInstr(NULL, "xor", TmpRegName(Expr->Reg),
                          TmpRegName(reg_one), TmpRegName(Expr->Reg)));
...
doBoolOp(struct ExprRes *Res1, struct ExprRes *Res2, int op)
{
    ...
    struct InstrSeq *instrs;
    instrs = GenInstr(NULL, opc, TmpRegName(Res1->Reg), TmpRegName(Res1->Reg),
        TmpRegName(Res2->Reg));
